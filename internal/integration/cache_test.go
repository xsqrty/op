...
func TestCache(t *testing.T) {
	...
	getLabelById := orm.NewReturnableCache(
		op.Select().From(labelsTable).Where(op.Eq("ID", cache.Arg("id"))),
	)
	countLabelById := orm.NewReturnableCache(
		op.Select().From(labelsTable).Where(op.Eq("ID", cache.Arg("id"))),
	)
	delLabelById := orm.NewReturnableCache(
		op.Delete(labelsTable).Where(op.Eq("ID", cache.Arg("id"))),
	)
